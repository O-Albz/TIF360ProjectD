...
class ConvCVAE(nn.Module):
    def __init__(self, latent_size = 128, num_classes = 17, device = 'auto', beta = 0.01): #for spectro 128,163
        ...
        # Decoder
        self.decoder_fc = nn.Linear(latent_size + num_classes, 512 * 16 * 27)
        ...
        self.decoder_conv1 = nn.Conv2d(512, 256, kernel_size=3, stride=1, padding=(1,1))
        self.upsample1 = nn.Upsample(scale_factor=(2, 2), mode='bilinear', align_corners=True)
        self.decoder_batch_norm1 = nn.BatchNorm2d(256)
        self.decoder_conv2 = nn.Conv2d(256, 128, kernel_size=3, stride=1, padding=(1,1))
        self.upsample2 = nn.Upsample(scale_factor=(2, 2), mode='bilinear', align_corners=True)
        self.decoder_batch_norm2 = nn.BatchNorm2d(128)
        self.decoder_conv3 = nn.Conv2d(128, 64, kernel_size=3, stride=1, padding=(1,1))
        self.upsample3 = nn.Upsample(scale_factor=(2, 2), mode='bilinear', align_corners=True)
        self.decoder_batch_norm3 = nn.BatchNorm2d(64)
        self.decoder_conv4 = nn.Conv2d(64, 32, kernel_size=3, stride=1, padding=(1,1))
        self.upsample4 = nn.Upsample(scale_factor=(2, 2), mode='bilinear', align_corners=True)
        self.decoder_batch_norm4 = nn.BatchNorm2d(32)
        self.decoder_conv5 = nn.Conv2d(32, 2, kernel_size=3, stride=1, padding=(1,0))#, output_padding=(1,1)
        self.upsample5 = nn.Upsample(scale_factor=(2, 2), mode='bilinear', align_corners=True)
...
# Discriminator will learn offset of mean. 
    def unFlatten(self, x):
        return x.reshape(x.shape[0], 512, 16, 27)
    ...
    def decoder(self, z):
        xdot = self.decoder_conv1(z)
        xdot = self.decoder_batch_norm1(xdot)
        xdot = F.leaky_relu(xdot)
        # xdot = self.dropout_conv(xdot)
        xdot = self.upsample1(xdot)

        xdot = self.decoder_conv2(xdot)
        xdot = self.decoder_batch_norm2(xdot)
        xdot = F.leaky_relu(xdot)
        xdot = self.upsample2(xdot)

        xdot = self.decoder_conv3(xdot)
        xdot = self.decoder_batch_norm3(xdot)
        xdot = F.leaky_relu(xdot)
        # xdot = self.dropout_conv(xdot)
        xdot = self.upsample3(xdot)

        xdot = self.decoder_conv4(xdot)
        xdot = self.decoder_batch_norm4(xdot)
        xdot = F.leaky_relu(xdot)
        xdot = self.upsample4(xdot)

        xdot = self.decoder_conv5(xdot)
        xdot = self.upsample5(xdot)

        xdot = F.interpolate(xdot, size=(513, 862), mode="bilinear", align_corners=False)
    ...
    def forward(self, x, labels):
        ...
        z = z.view(-1, 512, 16, 27)     # shape: [B, 512, 16, 26]
    ...
    def loss_function(self, recon_x, x, mu, logvar, alpha = 1):
        BCE = F.l1_loss(recon_x, x, reduction='sum') / x.size(0)
